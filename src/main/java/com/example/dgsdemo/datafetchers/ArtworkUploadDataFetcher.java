...
public class ArtworkUploadDataFetcher {
   ...
   @DgsMutation
    public List<Image> addArtwork(@InputArgument Integer showId, @InputArgument MultipartFile upload) throws IOException {
        ...
        Path newFile = uploadDir.resolve("show-" + showId + "-" + UUID.randomUUID() + Objects.requireNonNull(upload.getOriginalFilename()).substring(upload.getOriginalFilename().lastIndexOf(".")));
